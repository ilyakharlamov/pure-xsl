...
<xsl:stylesheet 
    ...
    exclude-result-prefixes="exslt unit"
    version="1.0">
    ...
    <xsl:template name="unit:testParseContent">
        <xsl:if test="true()">
            <xsl:call-template name="pxml:parseContent">
                <xsl:with-param name="escaped"></xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="true()">
            <xsl:call-template name="pxml:parseContent">
                <xsl:with-param name="escaped">     &lt;field attr1 = "value1" attr2 = 'value2'&gt;&lt;name/&gt;&lt;/field&gt;&lt;field&gt;te&amp;lt;xt&lt;/field&gt;</xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="pxml:parseContent">
        ...
        <xsl:message>escaped:<xsl:value-of select="$escaped"/>:escaped</xsl:message>
        <xsl:if test="$length > 0">
            ...
            <xsl:message>elementname:<xsl:value-of select="$elementname"/>:elementname</xsl:message>
            <xsl:choose>
                <xsl:when test="string-length($elementname)"><!-- element -->
                    <xsl:variable name="tagcontent">
                        <xsl:call-template name="getTagContent">
                            <xsl:with-param name="text" select="$escaped"></xsl:with-param>
                        </xsl:call-template>
                    </xsl:variable>
                ...
                <xsl:otherwise><!-- text -->
                    ...
                    <xsl:call-template name="replaceAmp">
                        <xsl:with-param name="text">
                            <xsl:choose>
                                <xsl:when test="$ltposition=0"><xsl:value-of select="$escaped"/></xsl:when>
                                <xsl:otherwise>
    ...
    <xsl:template name="replaceAmp">
        <xsl:param name="text"/>
        <xsl:message>RRR:<xsl:value-of select="$text"/>:RRR</xsl:message>
        <xsl:choose>
    ...
    <xsl:template name="unit:testGetElementName">
        <xsl:if test="true()">
            <xsl:call-template name="unit:assert">
                ...
                <xsl:with-param name="actual">
                    <xsl:call-template name="getElementName">
                        <xsl:with-param name="text">&lt;tag /&gt;</xsl:with-param>
                    </xsl:call-template>
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        ...
        <xsl:if test="true()">
            <xsl:call-template name="unit:assert">
                ...
                <xsl:with-param name="actual">
                    <xsl:call-template name="getElementName">
                        <xsl:with-param name="text">&lt;tag name="fu"&gt;&lt;tag&gt;</xsl:with-param>
                    </xsl:call-template>
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template name="unit:assert">
        ...
        <xsl:if test="$expected!=$actual">FAILED expected:<xsl:value-of select="$expected"/> actual:<xsl:value-of select="$actual"/></xsl:if>
    </xsl:template>
